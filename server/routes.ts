...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Experience Addon routes
  app.get('/api/experiences/:id/addons', isAuthenticated, async (req, res) => {
    try {
      const experienceId = parseInt(req.params.id);
      ...
      res.status(500).json({ message: 'Failed to fetch experience addons' });
    }
  });
  
  app.post('/api/experience-addons', isAuthenticated, hasRole('admin'), async (req, res) => {
    try {
      ...
      await storage.createActivity({
        userId: req.user?.claims?.sub || '0',
        action: 'Created experience addon',
        details: { 
          experienceId: addon.experienceId, 
          addonId: addon.id,
          name: addon.name
        }
      });
      ...
      console.error('Error creating experience addon:', error);
      
      if (error instanceof z.ZodError) {
        return res.status(400).json({ message: 'Invalid data', errors: error.errors });
      }
      
      res.status(500).json({ message: 'Failed to create experience addon' });
    }
  });
  
  app.patch('/api/experience-addons/:id', isAuthenticated, hasRole('admin'), async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      ...
      if (experience) {
        ...
        await storage.updateExperience(updatedAddon.experienceId, {
          addons: updatedAddons
        });
      }
      ...
      await storage.createActivity({
        userId: req.user?.claims?.sub || '0',
        action: 'Updated experience addon',
        details: { 
          experienceId: updatedAddon.experienceId,
      ...
      console.error('Error updating experience addon:', error);
      
      if (error instanceof z.ZodError) {
        return res.status(400).json({ message: 'Invalid data', errors: error.errors });
      }
      
      res.status(500).json({ message: 'Failed to update experience addon' });
    }
  });
  
  app.delete('/api/experience-addons/:id', isAuthenticated, hasRole('admin'), async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      ...
      const experienceId = addon.experienceId;
      ...
      await storage.createActivity({
        userId: req.user?.claims?.sub || '0',
        action: 'Deleted experience addon',
        details: { 
          experienceId: addon.experienceId,
          addonId: id,
          name: addon.name
      ...
      res.status(200).json({ message: 'Experience addon deleted successfully' });
    } catch (error) {
      console.error('Error deleting experience addon:', error);
      res.status(500).json({ message: 'Failed to delete experience addon' });
    }
  });
